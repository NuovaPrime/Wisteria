...
namespace Wisteria.Common.Loaders
{
    internal static class Loader
    {
        internal static void Autoload()
        {
            ...
            foreach (Mod mod in ModLoader.Mods)
            {
                if (mod.Code != null)
                {
                    ...
                    {
                        // Don't attempt to autoload abstracts (duh) and don't attempt to load anything that doesn't have a default constructor.
        ...
        {
            foreach (SlayerRank slayerRank in SlayerRankLoader.slayerRanks.Values)
            {
                slayerRank.AutoStaticDefaults();
                slayerRank.SetStaticDefaults();
            }
        }
